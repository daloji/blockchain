...
@PrepareForTest({LevelDbProxy.class,DeserializerTrame.class})
public class DeserializerTrameTest {
	...
	//@Test
	public  void  deserialiseTestOK() {
		EasyMock.expect(LevelDbProxy.getInstance()).andReturn(leveldb);
	...
	/**
	 * Verification Deserialisation BlockTrame
	 */

	@Test
	public void deserialise010() {
		PeerNode peer = new PeerNode(IPVersion.IPV4);
		peer.setHost("127.0.0.1");
		peer.setPort(8333);
	...
	@Test
	public void deserialise017() {

		PeerNode peer = new PeerNode(IPVersion.IPV4);
		peer.setHost("127.0.0.1");
		peer.setPort(8333);
		ArrayDeque<TrameHeader> stackCommand = DeserializerTrame.getInstance().deserialise(null,Utils.hexStringToByteArray(trame_receive_0017), peer);
		Assert.assertNotNull(stackCommand); 
		Assert.assertEquals(stackCommand.size(), 1);
		InvTrame inv = (InvTrame) stackCommand.poll();
		Assert.assertEquals(inv instanceof InvTrame, true);
	...
	@Test
	public void deserialiseBlock003() {
