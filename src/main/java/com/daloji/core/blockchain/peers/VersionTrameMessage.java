 ...
 */
public class VersionTrameMessage  extends MessageHeader{
	...
	private String generatePayload() {
		...
		String ipdest = Utils.convertIPtoHex(addressTrans);
		logger.debug("adresseIP destination  :"+Utils.convertIPtoHex(addressTrans));	
	...
	public <T> T deserialise(byte[] msg) {
		...
		System.out.println(" info length" +msg.length);
		System.out.println(" info length" + Utils.bytesToHex(msg));;
		...
		if(msg.length-offset>0) {
			VersionTrameAck verak = new VersionTrameAck();
			buffer = new byte[msg.length-offset];
			System.arraycopy(msg,offset, buffer, 0, buffer.length);
			VersionTrameAck deserialiseVerak = verak.deserialise(buffer);
